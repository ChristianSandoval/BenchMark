...
public CustomerInfoExt findCustomerExtById(String id)
    ...
    public CustomerInfoExt findCustomerExt(String card) throws BasicException {
        ...
        return (CustomerInfoExt) new PreparedSentence(s
                , "SELECT ID, TAXID, SEARCHKEY, NAME, CARD, TAXCATEGORY, NOTES, MAXDEBT, VISIBLE, CURDATE, CURDEBT" +
                  ...
                  " FROM CUSTOMERS WHERE ID = ? AND VISIBLE = " + s.DB.TRUE()
                , SerializerWriteString.INSTANCE
    ...
    public final boolean isConsumoEmpleado(String id) throws BasicException {

        Calendar currentDate = Calendar.getInstance(); //Get the current date
        SimpleDateFormat formatter= new SimpleDateFormat("yyyy-MM-dd"); //format it as per your requirement
        String dateNow = formatter.format(currentDate.getTime());
        return new PreparedSentence(s,
                "SELECT C.ID FROM RECEIPTS R INNER JOIN TICKETS T ON T.ID=R.ID " +
                "INNER JOIN CUSTOMERS C ON C.ID=T.CUSTOMER " +
                "INNER JOIN PAYMENTS P ON P.RECEIPT=R.ID " +
                "WHERE R.DATENEW>='"+dateNow+" 00:00:00"+"' AND C.ID=? AND P.TOTAL=17.60",
                SerializerWriteString.INSTANCE,
                SerializerReadString.INSTANCE).find(id)
            != null;
    }
...
*/
    ...
    public final SentenceExec getCustomerUpdate() {
        ...
        return new PreparedSentence(s
                , "UPDATE CUSTOMERS SET CARD=? WHERE ID = ?"
                , SerializerWriteParams.INSTANCE);
    }
